...
def nash_welfare(xy):
    x = xy[:, 0]
    y = xy[:, 1]
    both_neg = np.logical_and(x < 0.0, y < 0.0)
    w = x * y
    w[both_neg] = -w[both_neg]
    return w
...
if USING_METRIC == 1:
    ...
    # (Mean, Std_err)
    raise ValueError("no values for NEGOTIATION with metric 2")
elif USING_METRIC == 3:
    # (Mean, Std_err)
    ...
    # (Mean, Std_err)
    negotiation_self_play = (0.7074, 0.0006)
    negotiation_cross_play = (0.6587, 0.0005)
    negotiation_same_play = (0.7014, 0.0006)
    negotiation_diff_play = (0.6160, 0.0007)
else:
    raise ValueError()
...
def _get_inputs():
    ...
    files_data = (
        ...
        File_data(
            ...
            "IAsymBoSandPD",
            20.0,
            "amTFT/2021_09_23/18_23_20/eval/2021_09_23/19_22_51"
            "/plot_self_crossself_and_cross_play_policy_reward_mean_player_col_vs_policy_reward_mean_player_row_matrix.json",
            "amTFT/2021_09_23/18_23_20/eval/2021_09_23/19_22_51"
            "/plot_same_and_diff_prefself_and_cross_play_policy_reward_mean_player_col_vs_policy_reward_mean_player_row_matrix.json",
            None,
            None,
            (((4.0, 1.0), (2.0, 2.0)), (-1.0, -1.0)),
            (UTILITARIAN_W, EGALITARIAN_W),
        ),
        File_data(
            "LOLA-Exact",
            "IAsymBoSandPD",
            200.0,
            "LOLA_Exact/2021_09_24/16_50_02/eval/2021_09_24"
            "/16_54_25"
            "/plot_self_crossself_and_cross_play_policy_reward_mean_player_col_vs_policy_reward_mean_player_row_matrix.json",
            "LOLA_Exact/2021_09_24/16_50_02/eval/2021_09_24/16_54_25"
            "/plot_same_and_diff_prefself_and_cross_play_policy_reward_mean_player_col_vs_policy_reward_mean_player_row_matrix.json",
            ASYMIBOS_MAX,
            ASYMIBOS_MIN,
            (((4.0, 1.0), (2.0, 2.0)), (-1.0, -1.0)),
...
def _get_stats(file_paths, n_players, file_data):
    if isinstance(file_paths, Final_values):
        ...
        if preference_path is not None:
            ...
            elif "diff pref vs diff pref" in perf_per_mode_bis.keys():
                diff_preferences_cross_play = perf_per_mode_bis[
                    "diff pref vs diff pref"
                ]
            else:
...
def _plot_bars_separate(perf_per_mode_per_files, welfare_split):
    ...
    rotation = 15
    plt.subplot(121)
    print("_plot_bars_separate 1st part")
    ...
    plt.subplot(122)
    print("_plot_bars_separate 2nd part")
...
def _preproces_values(all_perf):
    ...
    print("self_play", self_play)
    print("cross_play", cross_play)
    print("same_pref_perf", same_pref_perf)
    print("diff_pref_perf", diff_pref_perf)
