...
from submission.algos.alternating_offers.alt_offers_training import (
    AltOffersTraining,
    run_episode,
)
...
from submission.base_game_experiments.alternating_offers.cond_params import (
    cond_params_iter,
...
# root_path = '~/ray_results/cross_play_20K_episodes/2021_03_18/22_47_17'
# binding = False

# root_path = '~/ray_results/cross_play_20K_episodes_binding/2021_03_20
# /20_17_24'
# binding = True

# root_path = '~/ray_results/cross_play_20K_episodes_cheap/2021_03_24/05_30_06'
# binding = False

# root_path = '~/ray_results/cross_play_20K_episodes_binding/2021_03_23/12_37_37'
# binding = True

# root_path = '~/ray_results/cross_play_20K_episodes_binding_prosocial/2021_05_15/09_10_05'
# binding = True

# root_path = '~/ray_results/cross_play_20K_episodes_binding_prosocial/2021_05_20/01_53_31'
# binding = True

root_path = "~/ray_results/cross_play_20K_episodes_binding_prosocial/2021_05_31/23_44_35"
binding = True
...
# root_path = '~/ray_results/population_experiments/2021_04_20/14_01_26'
# cond_params_list = list(cond_params_iter_pop_training())

# root_path = '~/ray_results/cross_play_20K_episodes_best_response_fixed_agent_0/2021_05_03/00_35_50'
# root_path = '~/ray_results/cross_play_20K_episodes_best_response_fixed_agent_1/2021_05_03/09_05_24'
# cond_params_list = list(cond_params_iter())
...
def get_rewards(args):
    ...
    config = {
        ...
        "hidden_embedding_sizes": [
            agent_params[0]["hidden_embedding_size"],
            agent_params[1]["hidden_embedding_size"],
        ],
        "utility_types": [
            f"skew_{agent_params[0]['utilities_skew']:.1f}",
            f"skew_{agent_params[1]['utilities_skew']:.1f}",
        ],
        "prosociality_levels": [
            agent_params[0]["prosociality_level"],
            agent_params[1]["prosociality_level"],
        ],
        "fairness_coeffs": [
            agent_params[0]["fairness_coeff"],
    ...
    if isinstance(trial_indices[0], list):  # population training eval
        ...
        pop_is = (
            trial_indices[0][1],
            trial_indices[1][1],
        )  # indices of agents in the population for each selected trial
        agent_weights = [  # have to fetch them here, otherwise if I pass weights in args it slows multiprocessing down
            matches[agent_match_regimes[0]][trial_is[0]]["agents"][0][
                pop_is[0]
            ],
            matches[agent_match_regimes[1]][trial_is[1]]["agents"][1][
                pop_is[1]
            ],
        ]
    else:
    ...
    for i in range(eval_episodes):
        env_state = AltOffersEnv(
            batch_size=alt_offers.args["batch_size"],
            random_state=alt_offers.train_random_state,
        ...
        (
            ...
            num_turns,
            alive_masks,
            new_agent_forward_stats,
            new_arb_forward_stats,
        ) = run_episode(
            args=alt_offers.args,
            env_state=env_state,
            agent_models=alt_offers.agent_models,
            arbitrator_model=alt_offers.arbitrator_model
            if alt_offers.args["enable_arbitrator"]
            else None,
            render_flag=False,
            deterministic=False,
        ...
        for reward_name, val in items_list:
            if reward_name not in all_rewards:
                ...
                all_rewards[reward_name] = np.concatenate(
                    [all_rewards[reward_name], val.numpy()]
                )
...
for i, trial_name in enumerate(os.listdir(root_path)):
    ...
    if os.path.isdir(trial_path):
        cur_trial_info = {}
        cur_trial_info["params"] = pickle.load(
            open(os.path.join(trial_path, "params.pkl"), "rb")
        )
        ...
        for agent_i in range(2):
            cur_trial_info["agents"].append(
                state["agent%s" % agent_i]["model_state"]
            )  # either a single model or a list of population models
        trial_info.append(cur_trial_info)

        # disable for population training
        if (
            cur_trial_info["params"]["cond_params"] == default_cond_params
        ):  # if custom params == default params
            default_indices.append(
                len(trial_info) - 1
            )  # correspond to default_vs_default plays
...
# disable for population training
matches["default_vs_default"] = [
    trial_info[default_i] for default_i in default_indices
]
all_training_modes = list(
    set([trial["params"]["match_mode"] for trial in trial_info])
)
for mode in all_training_modes:
    matches[mode] = [
        trial for trial in trial_info if trial["params"]["match_mode"] == mode
    ]
...
for match_regime, match_trials in matches.items():
    ...
    if match_regime == "default_vs_default":
        match_indices["default_vs_default"] = [
            list(range(len(matches["default_vs_default"])))
        ...
        for cond_params_i, cond_params in enumerate(cond_params_list):
            eq_list = [
                (i, trial)
                for (i, trial) in enumerate(match_trials)
                if round_cond_params(trial["params"]["cond_params"])
                == cond_params
            ]
...
results_filename = os.path.join(
    "cross_play_evals",
    "results_" + "_".join(root_path.split("/")[-3:]) + ".pickle",
)
...
for agent_0_match_regime, agent_1_match_regime in (
    ...
    for cond_params_i in range(
        len(cond_params_list)
    ):  # over all types of agents
        ...
        if agent_0_match_regime == agent_1_match_regime:
            eval_trials_list = [
                (trial_0_i, trial_0_i) for trial_0_i in trial_0_indices
            ]  # to match agents trained together
        else:
            #             eval_trials_list = [(trial_0_i, trial_1_i) for trial_0_i in trial_0_indices for trial_1_i in trial_1_indices]
            eval_trials_list = [
                (trial_0_i, trial_1_i)
                for trial_0_i in trial_0_indices[:5]
                for trial_1_i in trial_1_indices[:5]
            ]  # let's limit this to avoid doing too much computation
                    ...
                    for pop_1_i in range(len(trial_1["agents"][1])):
                        args_list.append(
                            (
                                ...
                                cond_params_i,
                                ([trial_0_i, pop_0_i], [trial_1_i, pop_1_i]),
...
chunk_size = 1024
chunks = [
    args_list[x : x + chunk_size] for x in range(0, len(args_list), chunk_size)
]
for chunk in chunks:
    cur_results = process_map(get_rewards, chunk)
    cur_results = [
        chunk[i][:3] + (result,) for i, result in enumerate(cur_results)
    ]
