...
from submission.utils import miscellaneous, log
...
class CrossPlayTraining(AltOffersTraining):
    def setup(self, config):
        ...
        if (
            config["match_mode"][-2] == "_"
            and config["match_mode"][-1].isdigit()
        ):
            config["match_mode"] = config["match_mode"][:-2]
        modes = config["match_mode"].split("_vs_")
        for mode in modes:
            if mode == "custom":
                config["utility_types"].append(
                    f'skew_{config["cond_params"]["utilities_skew"]}'
                )
                config["prosociality_levels"].append(
                    config["cond_params"]["prosociality_level"]
                )
                config["hidden_embedding_sizes"].append(
                    config["cond_params"]["hidden_embedding_size"]
                )
                config["fairness_coeffs"].append(
                    config["cond_params"]["fairness_coeff"]
                )
            elif mode == "default":
                config["utility_types"].append(
                    f'skew_{default_cond_params["utilities_skew"]}'
                )
                config["prosociality_levels"].append(
                    default_cond_params["prosociality_level"]
                )
                config["hidden_embedding_sizes"].append(
                    default_cond_params["hidden_embedding_size"]
                )
                config["fairness_coeffs"].append(
                    default_cond_params["fairness_coeff"]
                )
                ...
                config["fairness_coeffs"].append(
                    [0.0, config["cond_params"]["fairness_coeff"]]
                )
