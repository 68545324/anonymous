...
from submission.algos.lola.utils import flatgrad
...
def corrections_func(
    ...
    if cube is not None:
        ac_logp0 = tf.reshape(
            mainPN[0].log_pi_action_bs_t, [batch_size, 1, trace_length]
        )
        ac_logp1 = tf.reshape(
            mainPN[1].log_pi_action_bs_t, [batch_size, trace_length, 1]
        )
        mat_1 = tf.reshape(
    ...
    # wt mem_efficient
        ...
        ac_logp0 = tf.reshape(
            mainPN[0].log_pi_action_bs_t, [batch_size, trace_length]
        )
        ac_logp1 = tf.reshape(
            mainPN[1].log_pi_action_bs_t, [batch_size, trace_length]
        )
        ...
        for i in range(1, trace_length):
            ...
            mat_cumsum = tf.add(
                mat_cumsum, ac_logp0_cumsum[i] * ac_logp1[:, i]
            )
            mat_cumsum = tf.add(
                mat_cumsum, ac_logp1_cumsum[i] * ac_logp0[:, i]
            )
    ...
    if corrections:
        ...
        if clip_lola_correction_norm:
            second_order0 = tf.clip_by_norm(
                second_order0, clip_lola_correction_norm, axes=None, name=None
            )
            second_order1 = tf.clip_by_norm(
                second_order1, clip_lola_correction_norm, axes=None, name=None
            )
        if clip_lola_actor_norm:
            v_0_grad_theta_0 = tf.clip_by_norm(
                v_0_grad_theta_0, clip_lola_actor_norm, axes=None, name=None
            )
            v_1_grad_theta_1 = tf.clip_by_norm(
                v_1_grad_theta_1, clip_lola_actor_norm, axes=None, name=None
            )
        ...
        if clip_lola_update_norm:
            delta_0 = tf.clip_by_norm(
                delta_0, clip_lola_update_norm, axes=None, name=None
            )
            delta_1 = tf.clip_by_norm(
                delta_1, clip_lola_update_norm, axes=None, name=None
            )
...
def simple_actor_training_func(
    policy_network, opp_policy_network, batch_size, trace_length, cube=None
):
    # not mem_efficient
    if cube is not None:
        ac_logp0 = tf.reshape(
            policy_network.log_pi_action_bs_t, [batch_size, 1, trace_length]
        )
        ac_logp1 = tf.reshape(
        ...
        v_0 = tf.matmul(
            tf.reshape(
                policy_network.sample_reward, [batch_size, trace_length, 1]
            ),
            mat_1,
        )
        v_0 = tf.reshape(
        ...
        v_1 = tf.matmul(
            tf.reshape(
                opp_policy_network.sample_reward, [batch_size, trace_length, 1]
            ),
            mat_1,
        )
        v_1 = tf.reshape(
    ...
    # wt mem_efficient
        ...
        ac_logp0 = tf.reshape(
            policy_network.log_pi_action_bs_t, [batch_size, trace_length]
        )
        ac_logp1 = tf.reshape(
            opp_policy_network.log_pi_action_bs_t, [batch_size, trace_length]
        )
        ...
        for i in range(1, trace_length):
            ...
            v_1 = tf.add(
                v_1, mat_cumsum * opp_policy_network.sample_reward[:, i]
            )
        v_0 = 2 * tf.reduce_sum(v_0) / batch_size
        v_1 = 2 * tf.reduce_sum(v_1) / batch_size
    ...
    v_0_pi_0 = (
        ...
        * tf.reduce_sum(
            ...
            * policy_network.log_pi_action_bs_t
        )
        / batch_size
    )
