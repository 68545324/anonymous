...
class ExactLOLA(object):
    ...
    def _build_update(self):
        ...
        self.v_1 = tf.matmul(
            tf.matmul(tf.matrix_inverse(I_m_P), self.R_1),
            s_0,
            transpose_a=True,
        )
        self.v_2 = tf.matmul(
            tf.matmul(tf.matrix_inverse(I_m_P), self.R_2),
            s_0,
            transpose_a=True,
        )
        ...
        if self.enable_corrections:
            grad_v_2_theta_2_theta_1 = tf.concat(
                [
                    tf.reshape(
                        tf.gradients(
                            tf.gradients(self.v_2, self.theta)[0][i][0],
                            self.theta_opp,
                        ),
                        [1, theta_size],
                    )
            ...
            second_order_1 = tf.transpose(
                tf.matmul(
                    tf.reshape(grad_v_1_theta_2, [1, theta_size]),
