...
import torch
from submission.algos.alternating_offers.envs.alt_offers_env_sampling import (
    generate_batch,
)
...
class AltOffersEnv(object):
    ...
    def __init__(self, batch_size, random_state, utility_types):
        ...
        batch_properties = generate_batch(
        ...
        self.max_batch_size = self.N.size()[
            0
        ]  # actual batch size will be smaller on some steps because games end early
        self.utilities = torch.zeros(self.max_batch_size, 2, 3).long()
        self.utilities[:, 0, :] = batch_properties["utilities"][
            0
        ]  # dimensions: batch size, player_i, item_i
...
class AltOffersEnvMemory(object):
    ...
    def update_sieve(self, response):
        ...
        self.sieve.set_dead_global(
            self.env_state.cur_num_turns, t + 1
        )  # assign a length of t+1 to those games that have just ended
        all_dead = (
            self.sieve.all_dead()
        )  # binary, if all games have ended or not; used to stop the episode rollout
...
def calc_rewards(
    ...
    if exceeded_pool.max() > 0:
        reward_eligible_mask[
            exceeded_pool.nonzero().long().view(-1)
        ] = 0  # set the finishing flags on these games
        if reward_eligible_mask.max() == 0:
    ...
    if not enable_overflow:
        ...
        )  # (pool-proposal) is for the other player
    else:
        proposal[:, acceptor] = cur_proposal
    max_utility, _ = s.utilities.max(
        1
    )  # maximum possible utility for each item, take max over players
    ...
    for b in reward_eligible_idxes:  # iterate over indices of finished games
        ...
        max_agreement_rewards = (
            []
        )  # only used if the enable_overflow variant flag is True
        ...
        for player_i in range(2):
            ...
            if not enable_overflow:
                ...
                for item_i in range(3):
                    if (
                        cur_proposal[b, item_i] + prev_proposal[b, item_i]
                        <= s.pool[b, item_i]
                    ):
                        raw_rewards[player_i] += (
                            proposal[b, player_i, item_i].item()
                            * s.utilities[b, player_i, item_i].item()
                        )
                        # only count items where an agreement was reached in the maximum theoretical rewards sum
                        max_agreement_r += (
                            s.pool[b, item_i].item()
                            * s.utilities[b, player_i, item_i].item()
                        )

            max_theoretical_r = (
                s.utilities[b, player_i].cpu().dot(s.pool[b].cpu())
            )  # reward for player_i assuming that it got all of the pool
            max_theoretical_rewards.append(
                max_theoretical_r.item()
            )  # suppose agent i has all items from the pool
            max_agreement_rewards.append(max_agreement_r)

        for i in range(2):
            if max_theoretical_rewards[i] != 0:
                scaled_rewards[i] = (
                    raw_rewards[i] / max_theoretical_rewards[i]
        ...
        rewards_final["difference_in_share_of_max"][b] = (
            rewards_final["player0_share_of_max"][b]
            - rewards_final["player1_share_of_max"][b]
        ).abs()
        ...
        if max_prosocial != 0:
            ...
            rewards_final["sum_share_of_max"][b] = (
                rewards_final["sum_raw"][b] / max_prosocial
            )
        ...
        if enable_decision is not None:
            ...
            rewards_final["player0_after_arb"][b] = rewards_final[
                "player0_share_of_max"
            ][b]
            rewards_final["player1_after_arb"][b] = rewards_final[
                "player1_share_of_max"
            ][b]
            ...
            if enable_decision[b] == 1:
                ...
                # (=> 0/1 rewards after redistribution)
                    ...
                    favored_i = (
                        1 - favor_decision[b]
                    )  # index of the player who to redistribute the rewards to
                    if (
                        max_theoretical_rewards[favored_i] == 0
                    ):  # if dot product of utilities and pool item numbers is 0:
                        ...
                        max_r = (
                            max_agreement_rewards[favored_i]
                            / max_theoretical_rewards[favored_i]
                        )
                    if favored_i == 0:  # if redistributing to player 0
                        ...
                        rewards_final["player1_arb_induced_part"][b] = (
                            max_r - rewards_final["player1_share_of_max"][b]
                        )  # => =max_r
                    else:
                        raise Exception
