...
import submission.algos.alternating_offers.nets as nets
import submission.algos.alternating_offers.logging_utils as logging_utils
from submission.algos.alternating_offers.envs.alt_offers_env import (
    AltOffersEnv,
    AltOffersEnvMemory,
    calc_rewards,
)
from submission.algos.alternating_offers.envs.alt_offers_env_sieve import (
    SievePlayback,
)

# from submission.base_game_experiments.alternating_offers.cond_params import cond_params_iter, round_cond_params  # for training with frozen params of one agent
...
class AltOffersTraining(tune.Trainable):
    def setup(self, args):
        ...
        if args["train_seed"] is not None:
            np.random.seed(
                args["train_seed"]
            )  # not sure it would init ALL the seeds, check if use
        ...
        #         restore_weights_dir = '~/ray_results/cross_play_20K_episodes_binding/2021_03_23/12_37_37'
            ...
            if isinstance(args["hidden_embedding_sizes"][agent_i], list):
                ...
                cur_pop = [
                    nets.AgentModel(
                        ...
                        hidden_embedding_size=args["hidden_embedding_sizes"][
                            agent_i
                        ][pop_i],
                    )
                ...
                if args["enable_cuda"]:
                    model.cuda()
                if (
                    self.fixed_agent_i is not None
                    and agent_i == self.fixed_agent_i
                ):
                    model.load_state_dict(self.fixed_agent_weight)
                self.agent_models.append(model)
                if (
                    self.fixed_agent_i is not None
                    or agent_i != self.fixed_agent_i
                ):
                    if args["agents_sgd"]:
                        self.agent_opts.append(
                            optim.SGD(
                                self.agent_models[agent_i].parameters(),
                                lr=1e-3,
                                weight_decay=1e-4,
                            )
                        )
                        ...
                        self.agent_opts.append(
                            optim.Adam(
                                params=self.agent_models[agent_i].parameters()
                            )
                        )
                else:
        ...
        if args["enable_arbitrator"]:
            assert (
                len(args["hidden_embedding_sizes"]) > 2
            )  # need an embedding size for the arbitrator too then
            self.arbitrator_model = nets.ArbitratorModel(
                entropy_reg=args["arbitrator_entropy_reg"],
                share_utilities=args["share_utilities"],
                hidden_embedding_size=args["hidden_embedding_sizes"][2],
            )
            if args["enable_cuda"]:
                self.arbitrator_model.cuda()
            if args["arbitrator_sgd"]:
                self.arbitrator_opt = optim.SGD(
                    self.arbitrator_model.parameters(),
                    lr=1e-3,
                    weight_decay=1e-4,
                )
            else:
                self.arbitrator_opt = optim.Adam(
                    params=self.arbitrator_model.parameters()
                )
        ...
        for cur_episode_i in range(self.args["episodes_per_step"]):
            ...
            for cur_batch_i in range(batches_num):
                ...
                if isinstance(self.agent_models[0], list):
                    ...
                    cur_batch_models = [
                        self.agent_models[0][index_0],
                        self.agent_models[1][index_1],
                    ]
                    cur_batch_opts = [
                        self.agent_opts[0][index_0],
                        self.agent_opts[1][index_1],
                    ]
                    embedding_sizes = [
                        self.args["hidden_embedding_sizes"][0][index_0],
                        self.args["hidden_embedding_sizes"][1][index_1],
                    ]
                else:
                ...
                if index_0 is None:
                    env_state = AltOffersEnv(
                        batch_size=self.args["batch_size"],
                        random_state=self.train_random_state,
                        utility_types=self.args["utility_types"],
                    )
                else:
                    env_state = AltOffersEnv(
                        batch_size=self.args["batch_size"],
                        random_state=self.train_random_state,
                        utility_types=[
                            self.args["utility_types"][0][index_0],
                            self.args["utility_types"][1][index_1],
                        ],
                    )
                ...
                (
                    ...
                    self.arb_logprobs_accum,
                    arb_entropy_loss,
                    rewards,
                    num_turns,
                    alive_masks,
                    self.new_agent_forward_stats,
                    self.new_arb_forward_stats,
                ) = run_episode(
                    args=self.args,
                    env_state=env_state,
                    ...
                    render_flag=(
                        cur_episode_i == self.args["episodes_per_step"] - 1
                    )
                    and not self.args["suppress_output"],
                    deterministic=False,
                    embedding_sizes=embedding_sizes,
                )

                for agent in [0, 1]:
                    for key, val in self.new_agent_forward_stats[
                        agent
                    ].items():
                ...
                if self.baseline is None:
                    self.baseline = {
                        name: self.type_constr.FloatTensor(
                ...
                baselined_rewards = {
                    name: rewards[name] - self.baseline[name]
                    for name in rewards
                }

                sgd_decay_episodes = self.args["training_episodes"]
                phase = min(
                    float(self.episodes_completed) / sgd_decay_episodes, 1
                )
                ...
                if self.args["agents_sgd"]:
                    for i in range(2):
                        for group in cur_batch_opts[i].param_groups:
                            group["lr"] = cur_sgd_lr
                if (
                    self.args["enable_arbitrator"]
                    and self.args["arbitrator_sgd"]
                ):
                ...
                if self.args["enable_arbitrator"]:
                    ...
                    welfare_grads = []
                    sieve_playback = SievePlayback(
                        alive_masks, enable_cuda=self.args["enable_cuda"]
                    )
                    for (
                        t,
                        global_idxes,
                        ...
                        sieve_playback
                    ):  # iterate over which games were active on every step (for each batch elem)
                        agent = t % 2
                        for action in action_logprobs[
                            t
                        ]:  # iterate over 7 action_logprobs: 1xresponse, 3xproposal, 3xproposal_repr
                            orig_welfare_reward = (
                                baselined_rewards["sum_share_of_max"][
                                    global_idxes
                                ]
                                .float()
                                .contiguous()
                            ...
                            orig_welfare_reward_by_agent[agent] += (
                                action * Variable(orig_welfare_reward)
                            ).sum()
                    for i in range(2):
                        welfare_grads.append(
                            torch.autograd.grad(
                                orig_welfare_reward_by_agent[i],
                                cur_batch_models[i].parameters(),
                                retain_graph=True,
                            )
                        )
                    ...
                    for (
                        ...
                        for action in action_logprobs[
                            ...
                            reward_name = (
                                "player0_arb_induced_part"
                                if agent == 0
                                else "player1_arb_induced_part"
                            )
                            # OK to use baselines for the induced part? Yeah I think this should be an unbiased gradient estimate
                            induced_agent_reward = (
                                baselined_rewards[reward_name][global_idxes]
                                .float()
                                .contiguous()
                                .view(sieve_playback.batch_size, 1)
                            )
                            cur_arb_logprobs = self.arb_logprobs_accum.sum(1)[
                                global_idxes
                            ].view(sieve_playback.batch_size, 1)
                            induced_reward_by_agent[agent] += (
                                action
                                * cur_arb_logprobs
                                * Variable(induced_agent_reward)
                            ).sum()
                    for i in range(2):
                        ...
                        cur_grads = torch.autograd.grad(
                            induced_reward_by_agent[i]
                            - entropy_loss_by_agent[i],
                            cur_batch_models[i].parameters(),
                            retain_graph=True,
                            create_graph=True,
                        )
                    ...
                    for agent_i in range(2):
                        for grad_i in range(len(welfare_grads)):
                            ...
                            arb_main_loss -= (
                                welfare_grads[agent_i][grad_i]
                                * induced_grads[agent_i][grad_i]
                            ).sum()
                    cur_arb_main_loss_coeff = self.args[
                        "arbitrator_main_loss_coeff"
                    ]
                ...
                for (
                    ...
                    for action in action_logprobs[
                        ...
                        if index_0 is not None:
                            ...
                            cur_prosociality_level = self.args[
                                "prosociality_levels"
                            ][agent][pop_i]
                            cur_fairness_coeff = self.args["fairness_coeffs"][
                                agent
                            ][pop_i]
                        else:
                            cur_prosociality_level = self.args[
                        ...
                        assert (
                            self.args["scale_before_redist"]
                            or cur_prosociality_level == 0
                        )
                        ...
                        assert (
                            not self.args["enable_arbitrator"]
                            or cur_fairness_coeff == 0
                        )
                        total_agent_reward = (
                            (1 - cur_prosociality_level)
                            * baselined_rewards[reward_name][global_idxes]
                            + cur_prosociality_level
                            * baselined_rewards["sum_share_of_max"][
                                global_idxes
                            ]
                            - cur_fairness_coeff
                            * baselined_rewards["difference_in_share_of_max"][
                                global_idxes
                            ]
                        )
                        ...
                        main_loss_by_agent[agent] += -(
                            action * Variable(total_agent_reward)
                        ).sum()
                ...
                for i in range(2):
                    ...
                    self.main_loss_by_agent_accum[i] += main_loss_by_agent[
                        i
                    ].item()
                    self.entropy_loss_by_agent_accum[
                        i
                    ] += entropy_loss_by_agent[i].item()

                self.num_turns_accum += num_turns.sum()
                for name in rewards:
                    self.rewards_sum[name] += rewards[name].sum()
                    self.baseline[name] = (
                        0.7 * self.baseline[name] + 0.3 * rewards[name].mean()
                    )
    ...
    def save_checkpoint(self, tmp_checkpoint_dir):
        ...
        for i in range(2):
            ...
            if isinstance(self.agent_models[i], list):
                state["agent%s" % i]["model_state"] = [
                    model.state_dict() for model in self.agent_models[i]
                ]
                state["agent%s" % i]["opt_state"] = [
                    opt.state_dict() if opt is not None else None
                    for opt in self.agent_opts[i]
                ]
            else:
                state["agent%s" % i]["model_state"] = self.agent_models[
                    i
                ].state_dict()
                state["agent%s" % i]["opt_state"] = (
                    self.agent_opts[i].state_dict()
                    if self.agent_opts[i] is not None
                    else None
                )
        ...
        for agent_i in range(2):
            if isinstance(state["agent%s" % i]["model_state"], list):
                for pop_i in range(len(self.agent_models[i])):
                    self.agent_models[agent_i][pop_i].load_state_dict(
                        state["agent%s" % agent_i]["model_state"][pop_i]
                    )
                    if (
                        state["agent%s" % agent_i]["opt_state"][pop_i]
                        is not None
                    ):
                        self.agent_opts[agent_i][pop_i].load_state_dict(
                            state["agent%s" % agent_i]["opt_state"][pop_i]
                        )
            else:
                self.agent_models[agent_i].load_state_dict(
                    state["agent%s" % agent_i]["model_state"]
                )
                if state["agent%s" % agent_i]["opt_state"] is not None:
                    self.agent_opts[agent_i].load_state_dict(
                        state["agent%s" % agent_i]["opt_state"]
                    )
...
def run_episode(
    ...
    env_agents_state = AltOffersEnvMemory(
        env_state,
        msg_len=3,
        hidden_embedding_sizes=embedding_sizes,
        enable_arbitrator=(arbitrator_model is not None),
    ...
    alive_masks = (
        []
    )  # accumulator of indices of alive games on every iteration
    ...
    if arbitrator_model is None:
        ...
        arb_logprobs_accum = type_constr.FloatTensor(max_batch_size, 2).fill_(
            0
        )  # 2 decisions
    ...
    for env_state.t in range(10):
        ...
        logprobs_by_timestep.append(
            action_logprobs
        )  # all action log-likelihoods

        if (
            render_flag and env_agents_state.sieve.out_idxes[0] == 0
        ):  # as long as the game with index 0 is still alive, render it
        ...
        env_agents_state.update_message_proposal(
            new_message.detach(), new_proposal.detach()
        )
        ...
        if arbitrator_model is not None:
            ...
            (
                ...
                favor_decision,
                arb_entropy_loss,
                env_agents_state.arb_hidden_state,
                env_agents_state.arb_cell_state,
                cur_forward_stats,
            ) = arbitrator_model(
                env_state.pool,
                env_state.utilities[:, 0],
                env_state.utilities[:, 1],
                opponent_proposal,
                new_message,
                response,
                env_state.t,
            ...
            arb_logprobs_accum[env_agents_state.sieve.out_idxes] = torch.cat(
                arb_logprobs, 1
            )
