...
from submission.utils.miscellaneous import logger
from submission.utils import restore
...
class PolicyCallbacks(DefaultCallbacks):
    def on_episode_start(self, worker: "RolloutWorker", *args, **kwargs):
        self._call_method_from_policies(
            worker, "on_episode_start", *args, **kwargs
        )

    def on_episode_step(self, worker: "RolloutWorker", *args, **kwargs):
        self._call_method_from_policies(
            worker, "on_episode_step", *args, **kwargs
        )

    def on_episode_end(self, worker: "RolloutWorker", *args, **kwargs):
        self._call_method_from_policies(
    ...
    def on_sample_end(self, worker: "RolloutWorker", *args, **kwargs):
        self._call_method_from_policies(
    ...
    def _is_callback_implemented_in_policy(self, policy, callback_method):
        return hasattr(policy, callback_method) and callable(
            getattr(policy, callback_method)
        )
...
def merge_callbacks(*callbacks_class):
    ...
    class MergedCallBacks(DefaultCallbacks):
        def __init__(self):
            super().__init__()
            self.callbacks = [
                callback() if inspect.isclass(callback) else callback
                for callback in callbacks_class
            ]
