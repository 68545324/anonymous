...
from submission.utils.cross_play import evaluator
from submission.utils.cross_play.stats_summary import StatisticSummary
from submission.utils.plot import PlotHelper, PlotConfig
...
class SelfAndCrossPlayPlotter:
    ...
    def plot_results(
        ...
        self._one_mode_only = len(set([el[0] for el in metrics_per_mode])) == 1
        print("self._one_mode_only", self._one_mode_only)
    ...
    def _extract_performance_evaluation_points(
        ...
        if len(available_metrics_list) > 0:
            ...
            warnings.warn(
                f"self&cross player plotter: mode {mode} has no data points"
            )
    ...
    def _get_label(self, mode, group_pair_name):
        ...
        if self._suffix_needed(group_pair_name):
            ...
            if self._one_mode_only:
                label = " vs ".join(ordered_group_pair_name)
            else:
